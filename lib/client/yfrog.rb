...
module Yfrog
  ...
  class YfrogScraper
    ...
    def search_by_user(username, pages=4)
      ...
      search_uri = "http://yfrog.com/user/#{username}/photos"
      ...
      # for each thumbnail
      doc.xpath("//*[@id=\"media-results\"]").children do |row|
        ...
        # grab the pic link
        image_link = "http://yfrog.com/#{path}"
...
#       p = YfrogPicture.new(doc.xpath("//*[@id=\"media\"]/img/@src").to_s)
...
#        pics << p if p
  ...
  class YfrogPicture
    ...
    def initialize(link, do_download=true)
          ...
          @remote_path = link
          @local_path = "/tmp/yfrog_file_#{rand(1000000)}"
